...
function SuspectedAccounts() {
	...
	const columns = [
		...
		},
		{
			...
			flex: 1,
			renderCell: ({ row }) => {
				return <Button variant="contained" color="error" startIcon={<ImBin size="16px" />} onClick={() => {
					localStorage.setItem("suspects", JSON.stringify(usersList.filter((user) => user.accountNumber !== row.accountNumber)));
